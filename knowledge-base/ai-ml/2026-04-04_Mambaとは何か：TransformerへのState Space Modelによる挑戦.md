...
監査エージェントでは長大な監査調書・規程・ログを扱う場面があり、Mambaの線形スケーリングは100万トークン超の文書処理を現実的にする点で直接的な応用価値がある。LangGraphベースのReActエージェントでは各ステップの状態管理が重要だが、Mambaの選択的状態更新（動的な記憶・忘却制御）はエージェントの作業記憶設計のアーキテクチャ参考になる。ローカルLLMインフラ（RTX 3090）での運用においても、KVキャッシュ不要によるメモリ効率の改善はGPUメモリ制約下での長文脈推論を実現する観点で検討価値がある。
