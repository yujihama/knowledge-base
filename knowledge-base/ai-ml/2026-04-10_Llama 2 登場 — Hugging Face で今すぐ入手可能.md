...
Llama 2-ChatはRLHFで最適化されており、GRPO/RLAIFの比較基準として技術的に参照価値が高い。QLoRA＋SFTTrainerで監査ドメイン特化のInstruction Tuningが単一GPU上で実現可能なため、監査エージェントのローカルLLMバックエンド（RTX 3090）候補として具体的に検討できる。TGIとLangGraphを組み合わせることで、低レイテンシなローカル推論エンドポイントを持つReActエージェントを構築するベースとして活用できる。
