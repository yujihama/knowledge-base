...
監査ドキュメント（財務諸表、監査調書等）をVLMでパースする際の学習データ不足は実務上の共通課題であり、本手法は限られた実ドキュメントから多様な訓練サンプルを生成する有力な選択肢となる。LangGraphベースの監査エージェントがOCRやドキュメント読取りモジュールを持つ場合、そのファインチューニングデータ生成パイプラインとしてAlbumentations TextImageを組み込むことで、データ収集コストを抑えながらモデル精度を向上できる可能性がある。
