...
監査エージェントでマルチモーダルLLMを活用する場合（例：財務書類の図表や証跡画像の解析）、オブジェクト幻覚は誤った監査判断に直結するリスクがある。FLBはトレーニング不要かつ推論コストほぼゼロで幻覚を抑制できるため、LangGraphベースの監査エージェントに組み込むデコーディング戦略として即実用可能な候補となる。また、LLM-as-judgeフレームワークで視覚的証拠の評価精度を高める際にも、視覚グラウンディングの維持という観点から参考になる。
