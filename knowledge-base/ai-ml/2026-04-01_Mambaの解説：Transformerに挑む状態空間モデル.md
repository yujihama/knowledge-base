...
監査エージェント開発において長文書（監査調書、内部統制文書）の処理が課題になる場面でMambaのO(n)線形スケーリングは有力な選択肢となる。LangGraphベースのエージェントでコンテキスト長がボトルネックになる場合、Mambaベースのバックボーンへの置き換えやハイブリッド構成（Jamba等）を検討する際の理論的基盤として参照価値がある。また「状態＝過去の圧縮」という概念は、エージェントのメモリアーキテクチャ設計（何をいつ忘れるか）の議論とも直接対応するため、ReActループにおける状態管理の設計指針として活用できる。
