...
監査エージェント開発において、特定の監査用語や規制文書の文脈依存的な分類（例：「重要性」が財務的重要性を指すか組織的重要性を指すか）にSetFitを応用できる可能性がある。1クラスあたり15〜20件という少数データで高精度を実現できる点は、監査ドメインのようにラベル付きデータ収集が困難な領域に適している。また、ルールベース（知識ベース）とML分類器を組み合わせるアーキテクチャは、監査基準の明示的ルールとLLM判断を統合するエージェント設計と親和性が高い。
