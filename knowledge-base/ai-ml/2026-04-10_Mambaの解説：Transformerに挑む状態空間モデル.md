...
監査エージェント開発において、長期にわたる監査証跡や会話履歴を処理する際にMambaのO(n)線形スケーリングは直接的に有用。LangGraphベースのエージェントでは各ステップの文脈を保持するメモリ機構が課題になるが、MambaのSSMを状態管理層として統合することで、監査ログの長文脈処理をメモリ効率よく実現できる可能性がある。また「選択的記憶」の概念はReActエージェントの観察（Observation）から重要情報のみを抽出するfiltering機構の設計に示唆を与える。ローカルLLMインフラ（RTX 3090）での長文脈推論においても、KVキャッシュ削減によるVRAM節約効果は実用的なメリットになる。
