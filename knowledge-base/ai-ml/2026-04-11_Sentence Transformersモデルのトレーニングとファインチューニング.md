...
監査エージェントのRAGコンポーネントにおいて、監査調書・規制文書・内部統制記述書のドメイン固有埋め込みモデルをファインチューニングする際の実装手順として直接参照できる。特にCase2形式（監査証拠と対応する監査手続のペア）を使ったMultipleNegativesRankingLossによるファインチューニングは、ラベル付きデータが少ない実務環境でも機能しやすい。LangGraphの検索ノードに組み込む埋め込みモデルをドメイン適応させることで、汎用モデルより精度の高いセマンティック検索が実現できる。
