...
監査エージェントでは長い監査ログや規制文書（数万〜数十万トークン）を扱う場面が多く、Mambaの線形スケーリング特性はKVキャッシュOOMを回避しつつ長文コンテキストを処理するバックボーンとして検討価値がある。また選択的状態更新の仕組みは、LangGraphのエージェント状態管理（何を記憶し何を忘却するか）の設計思想と対応しており、エージェントメモリアーキテクチャの参照モデルとして有用。ローカルLLMインフラ（RTX 3090）での推論コスト削減にも、Mamba系モデルの5倍高速化は直接的な恩恵をもたらす。
