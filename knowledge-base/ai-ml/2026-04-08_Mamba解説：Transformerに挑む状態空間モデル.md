...
監査エージェントシステムでは大量の取引ログや監査証跡を長文コンテキストとして処理するニーズがあり、MambaのO(n)スケーリングは100万トークン規模のシーケンス処理を現実的にする点で注目に値する。LangGraphベースのエージェントにおけるコンテキスト管理の代替アーキテクチャとして、Mambaの選択的状態更新（何を記憶し何を忘れるかを入力依存で制御）はエージェントのメモリ機構設計に示唆を与える。ただし現時点でIn-Context Learningがやや弱いため、few-shot prompting依存のエージェント設計には慎重な評価が必要。
