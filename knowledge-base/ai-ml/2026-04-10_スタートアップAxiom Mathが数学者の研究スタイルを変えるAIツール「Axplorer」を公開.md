...
Axplorerの「人間がフィードバックを与えながらAIが候補を生成・改良するループ」という設計は、監査エージェントにおけるLLM-as-judgeやRLAIFの実装構造と概念的に近く、監査手続きの仮説生成・検証サイクルへの応用が考えられる。また、LLMは「既存事例の補間には強いが前例のない異常検知には弱い」というChartonの知見は、監査AIにおける異常パターン発見のアーキテクチャ選択（LLMだけに頼らず特化型探索モジュールを組み合わせる必要性）への示唆として参考になる。
