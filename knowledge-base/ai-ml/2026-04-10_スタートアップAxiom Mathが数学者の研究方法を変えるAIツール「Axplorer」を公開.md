...
監査エージェント開発において、既知ルールの適用（LLM的アプローチ）だけでなく、未知のリスクパターンを発見する探索的フレームワークが必要になる場面がある。AxplorerのようなヒューマンインザループでのパターンBoost手法は、監査データから新たな不正パターンや異常シグナルを発見するエージェント設計のヒントになる。また、ローカル環境（RTX 3090）での高効率探索という設計方針は、Yujiのローカルインフラ構築とも親和性が高い。
