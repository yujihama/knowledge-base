...
監査エージェントでLLM-as-judgeを使う際、証拠書類の順序や密度がスコア判定に影響するリスクは本記事の問題と同質。influence_mapのように「どの入力がスコアを主導したか」を構造化出力として記録する設計はPydanticのレスポンスモデルに組み込みやすく、監査ログの説明責任要件にも対応できる。LangGraphでのノード設計において、高密度なコンテキストを先に渡すノードと後続ノードでスコアがずれる現象への対処として、ローリングウィンドウ的な分割処理パターンも参考になる。
