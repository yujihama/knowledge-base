...
監査エージェント開発において、Axplorerの「例示→生成→選択→再入力」という反復的パターン探索ループは、LangGraphの状態遷移設計や監査手続きの仮説生成フローに応用できる。LLMが既存パターンを再利用する傾向（保守性）という指摘は、監査AI設計でLLMを使う際に「新規不正手口の発見」にはLLM単独では限界があることを示唆しており、パターン探索的な非LLMアルゴリズムとのハイブリッド構成を検討する根拠になる。
