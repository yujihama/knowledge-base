...
監査エージェントでは長い監査証跡・ドキュメント履歴を処理する必要があり、MambaのO(n)スケーリングは百万トークン規模の長文脈処理を現実的にする点で直接的な価値がある。LangGraphのノード間で受け渡す状態（State）の設計思想とSSMの隠れ状態設計は概念的に共鳴しており、「何を圧縮してStateとして保持すべきか」という設計判断に新たな視点を与える。ローカルLLMインフラ（RTX 3090）での運用においても、KVキャッシュ削減によるメモリ効率向上は実用上の恩恵が大きい。
