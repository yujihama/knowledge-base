...
監査エージェントにおいて長い監査証跡・法令文書・過去の監査調書を文脈として扱う場合、Transformerのメモリ制約がボトルネックになりうるが、MambaのO(1)推論特性はローカルLLMインフラ（RTX 3090）での長文脈処理コスト削減に直結する。ただしMambaの「損失圧縮」特性は、監査において特定条文の正確な参照や証跡の完全な再現が求められる場面では精度リスクとなるため、ハイブリッドアーキテクチャ（Jamba等）の検討が実用上有効。LangGraphエージェントのメモリ設計においても、SSMの「状態＝過去の圧縮」という概念はエージェントの作業記憶設計と対応しており、アーキテクチャ選定の理論的根拠として参照できる。
