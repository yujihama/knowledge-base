...
監査エージェントで扱うデータ（財務指標、KRI、外部環境変数など）も外生変数の欠損・ずれが頻発する。FTimeXerの確率的マスキング＋一貫性正則化というロバスト学習スキームは、不完全な外部データを入力とするリスク予測モジュールの設計に直接応用できる。ただし本論文の主テーマはカーボン強度予測であり、監査ドメインへの直接移転には追加検証が必要。
