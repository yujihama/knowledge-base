...
DriveVLMのChain-of-Thought推論（シーンを段階的に解析→行動を言語で説明）は、LangGraphで構築する監査エージェントのReAct推論ステップ設計と構造が近く、判断根拠の透明化手法として参考になる。またDriveLMのシーングラフを用いたQ&A駆動プランニングは、監査手続きをグラフ構造で表現しLLMに問い合わせるアーキテクチャのヒントになる。自動運転固有の課題（ハルシネーション、リアルタイム制約）は監査エージェントの信頼性設計でも共通の論点であり、LLM-as-judgeの評価設計に示唆を与える。
