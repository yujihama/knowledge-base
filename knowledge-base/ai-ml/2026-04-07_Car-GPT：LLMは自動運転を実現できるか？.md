...
LLMを意思決定エージェントとして活用する際のアーキテクチャ設計パターン（既存の特化モデルをエンコーダとして前段に置き、LLMが推論・判断を担う構成）は、監査エージェントでも応用可能。例えばリスク評価モデルや異常検知モデルの出力をトークン化し、LangGraphのノードとして接続する設計に直接転用できる。また、拡散モデルによる合成シナリオ生成は、監査向け学習データ（不正取引シナリオ等）の不足を補う手法として参考になる。
