...
監査エージェント開発において、複数の内部統制ルール・規制要件・リスク基準を同時に考慮する判断タスク（例：取引の異常検知、統制評価の矛盾検出）はExtended Thinkingが有効なタスク類型と合致する。思考ブロックが読める特性は、監査AIの判断根拠をトレーサブルにする説明責任要件を満たす手段として直接活用できる。LangGraphのノード設計においてタスク種別ごとにbudget_tokensを切り替えるルーティングノードを組み込むことで、精度とコストのバランスを最適化できる。
