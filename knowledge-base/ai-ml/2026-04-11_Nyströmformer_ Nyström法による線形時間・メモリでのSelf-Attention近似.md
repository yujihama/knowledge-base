...
監査エージェント開発において、長い監査報告書・規制文書・契約書をコンテキストとして処理する際にO(n²)のAttentionはメモリ上のボトルネックとなる。Nyströmformerのような線形Attentionをローカルインフラ（RTX 3090）上のLLMのベースとして採用することで、長文文書の効率的な処理が可能になる。また、RAGパイプラインのリランカーや文書埋め込みモデルとして組み込む際のアーキテクチャ選定基準として、この手法の計算複雑度トレードオフは重要な参照点となる。
