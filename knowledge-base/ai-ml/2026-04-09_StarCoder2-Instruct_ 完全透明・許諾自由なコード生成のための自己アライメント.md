...
監査エージェント開発において、GPT-4などの外部APIへの依存なしにドメイン特化のinstruction-tuningデータを自己生成する手法として直接応用可能。特に、監査手続きのコード生成タスクに対して実行ベースの自己検証パイプラインを組み込むことで、人手レビューコストを削減しつつデータ品質を担保できる。LangGraphやPydanticを使ったエージェントのSFT用合成データ生成にも、Self-OSS-InstructのコードコンセプトラベリングアプローチとGRPO/RLAIFとを組み合わせる設計が参考になる。
