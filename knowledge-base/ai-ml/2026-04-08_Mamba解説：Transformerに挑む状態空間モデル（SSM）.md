...
監査エージェント開発における長文書（監査調書・契約書・財務報告書）処理において、MambaのO(1)推論と線形スキャン特性は大量ドキュメントの逐次処理に有効な選択肢となりうる。LangGraphベースのエージェントでノード間の状態管理にSSMの「状態圧縮」コンセプトを応用すれば、エージェント間の文脈引き継ぎを効率化できる可能性がある。ただし監査AIでは説明可能性が重要なため、アテンションマップが使えないMambaの解釈性制約はLLM-as-judgeパターンとの組み合わせで補完する設計が必要になるだろう。
