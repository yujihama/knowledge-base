...
監査エージェント開発において長大な監査調書・契約文書・取引ログをコンテキストとして扱う場合、TransformerのO(n²)制約がボトルネックになり得るが、MambaのO(n)線形スケーリングはその解決策候補となる。LangGraphベースのエージェントのバックボーンLLMとしてMambaベースのモデルを採用することで、長期の監査証跡を低レイテンシで処理できる可能性がある。また、Selective State Spaceが「関連情報の選択的保持」を学習する仕組みは、監査で重要なリスク指標を文書から動的に抽出するLLM-as-judgeコンポーネントの設計思想として参考になる。
