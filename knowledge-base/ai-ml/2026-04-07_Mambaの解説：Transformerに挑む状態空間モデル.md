...
監査エージェントは長期にわたる監査証跡（取引ログ・コントロール記録）を処理する必要があり、Mambaの線形スケーリングは大量ドキュメントのシーケンシャル処理に直接応用できる。LangGraphベースのエージェントのバックボーンとしてMambaを採用すれば、Transformerでは現実的でなかった長い監査コンテキスト（全会計年度の仕訳データ等）を1パスで処理できる可能性がある。ただし隠れ状態への圧縮は情報の非可逆的損失を伴うため、監査における証跡の完全性・再現性要件との整合性検討が必要。GRPO/RLAIFによる強化学習との組み合わせでは、Mambaの高速推論が報酬モデルのサンプリング効率向上に寄与しうる。
