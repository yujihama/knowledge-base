...
監査エージェント開発においてLangGraphで長い監査ログやドキュメントチェーンを処理する場面で、Transformerの二次計算量がボトルネックになる可能性があり、MambaベースのバックボーンはRAGなしで100万トークン級の長文脈を扱う選択肢として検討価値がある。選択的SSMによる「何を記憶し何を捨てるか」の動的制御はReActエージェントの状態管理設計の参考になる。ローカルLLMインフラ（RTX 3090）でのMambaモデルのfine-tuningはGRPO/RLAIFの実験コストを下げる可能性もある。
