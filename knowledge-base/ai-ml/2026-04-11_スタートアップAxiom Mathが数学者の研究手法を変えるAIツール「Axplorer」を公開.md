...
Axplorerの「ユーザー選択→生成→再入力」のループ構造はReActエージェントやLangGraphの人間フィードバックノードと設計思想が共通しており、監査エージェントにおける仮説生成・絞り込みサイクルの参考になる。またLLMが既知パターンの派生には強いが未知の構造発見には弱いという知見は、監査異常検知においてLLMをスコアリングではなく探索補助として位置づける設計判断の根拠となり得る。
