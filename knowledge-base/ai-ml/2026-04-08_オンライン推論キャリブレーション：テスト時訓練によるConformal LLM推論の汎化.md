...
監査エージェントでは誤った判断への信頼度が問題となるが、ORCAのConformal Predictionベースのアプローチは「信頼できる回答のみを出力し、不確実な場合は追加サンプリングで補う」という制御に応用できる。LangGraphのノードごとに信頼度しきい値を設けてループ回数を動的制御する設計と相性がよく、LLM-as-judgeの精度維持と計算コスト削減を両立できる可能性がある。self-consistencyラベルのみで機能する点は、正解ラベルのない監査ユースケース（リスク評価の妥当性判定等）での実用化を示唆する。
