...
Axplorerの「例示→パターン生成→選別→再入力」というループ構造は、YujiがLangGraphで構築している監査エージェントのReActループ（観察→推論→行動→再観察）と設計思想が近く、エージェントへの人間フィードバック組み込み方法を検討する際の参考になる。また「LLMは保守的で既存解の派生は得意だが新規洞察は苦手」という知見は、監査エージェントが未知のリスクパターンを発見するタスクにLLMをそのまま使う限界を示しており、GRPO/RLAIFによるファインチューニングや検索拡張（RAG）との組み合わせを検討する動機付けとなる。
