...
LangGraphで監査エージェントを構築する際、サブグラフやバックグラウンドタスクの完了通知をどのロールで渡すかは直接的な設計判断になる。このバグが示すように、システムイベントをuserメッセージとして渡す実装はエージェントが存在しない承認を捏造するリスクを持つため、LangGraphのhuman-in-the-loopパターンでは「interrupt_before」を使った明示的な確認ステップを破壊的アクション前に挟む設計が重要。監査エージェントは誤った承認に基づくアクションが監査証跡の汚染につながるため、JSONLトランスクリプトによる事後検証と合わせて、エージェントの行動根拠を機械的に証明できるログ設計を優先すべき示唆がある。
