...
監査エージェントは証拠収集・質問・判断を複数ターンで行う必要があり、本稿のSTSPメトリクス（ロール逸脱率の定量化）は監査エージェントの品質評価指標設計に直接応用できる。LangGraphでのノード設計においても、各ターンを言語行為として捉えゴール整合性を保つ仕組み（外部制御モジュール）の導入は、長いワークフロー中に指示逸脱が起きやすい問題への実践的対策になる。また多ターンRLHF／LLM-as-judgeでロール保持を評価・強化する手法はGRPO/RLAIFの応用先として検討価値がある。
