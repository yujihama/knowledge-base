...
監査エージェントは単一ターンのQ&Aではなく、証拠収集・仮説検証・追加確認の多ターンプロセスであるため、Purposeful Dialogueの枠組みは直接適用できる。LangGraphのStateGraphで「現在の監査目標」「未解決の確認事項」「対話履歴」を状態として明示管理し、Role Driftを防ぐ設計が示唆される。また、RLHFの多ターン拡張はGRPO/RLAIFの研究に接続しており、監査判断の連続的な正確性を報酬として設計するアプローチの根拠として使える。
