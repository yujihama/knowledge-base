...
監査エージェント開発において、複数セッションにわたる調査履歴・判断根拠・規制解釈の変遷を失わずに蓄積・検索できる点が直接応用可能。LangGraphの各ノードからMCP経由でMemPalaceを呼び出し、Wing=監査案件・Room=リスク項目・Hall=発見事項/判断根拠という階層でエージェントの長期記憶を構造化する構成が考えられる。またTemporal Knowledge Graphの矛盾検出機能は、監査証跡において「誰が何時点でどのファクトを主張したか」を追跡するLLM-as-judgeパターンとも相性が良い。ローカル完結動作はDeloitte環境でのデータガバナンス要件に対応しやすい点も実用上の利点となる。
