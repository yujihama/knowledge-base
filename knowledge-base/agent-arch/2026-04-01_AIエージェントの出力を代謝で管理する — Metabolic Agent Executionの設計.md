...
監査エージェントでは証拠収集・判断・レポート生成の各ステップでLLM出力の信頼性保証が必須であり、Validator Ladder（L1〜L4）とrepair_chunkの設計はLangGraphの各ノード出力にそのまま適用できる。特にL3のbrief alignment checkはPydanticのスキーマ検証と組み合わせることで、監査基準との整合性チェックを自動化する実装パターンとして活用できる。ExecutionStateMemoryによる実行履歴の下流伝播は、LangGraphのStateオブジェクトで監査トレイルを蓄積する設計に直接対応する。
