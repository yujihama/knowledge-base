...
監査エージェントは単発Q&Aではなく、証拠収集→仮説検証→追加質問というマルチターン構造を持つため、purposeful dialogueの設計原則は直接適用可能。LangGraphのステートマシンでシステムプロンプトを各ノード遷移時に再注入することで、長期対話での指示追従崩壊を防ぐ実装が検討できる。また、RLAIF/RLHFの報酬設計において「監査目標の達成度（不正リスクの特定率など）」を長期報酬として定義する際の理論的根拠としても本稿の枠組みが活用できる。
