...
監査エージェントにおける「エージェントがいつ・どのように介入するか」の設計問題に直接応用できる。Sensible Agentのwhat/howの2段階分離設計は、LangGraphでの監査エージェント構築時にノード分割の参考になる（例：リスク検知ノードと報告形式選択ノードの分離）。また、コンテキストに応じて出力モダリティを動的選択する設計は、監査レポートの自動生成において読み手（CFO向け要約 vs 内部監査担当向け詳細）に応じたアウトプット形式の切り替えロジックに転用可能。
