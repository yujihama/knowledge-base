...
監査エージェントはエビデンス収集・質問・確認という複数ターンの往復を本質的に含むため、本稿のPurposeful Dialogue評価フレームワーク（ロールスワップ実験・LLM-as-judge）はLangGraphで構築中のエージェントの一貫性評価に直接応用できる。特に『長期対話でシステムプロンプトの監査役割を維持できるか』というストレステストは、ReActループが長引いた際の挙動検証に有効。また、発話を『相手の世界モデルを更新する行為』として定式化する視点は、監査エージェントが被監査部門担当者との対話戦略を設計する際のPydanticスキーマ設計（goal state・hidden intent）に示唆を与える。
