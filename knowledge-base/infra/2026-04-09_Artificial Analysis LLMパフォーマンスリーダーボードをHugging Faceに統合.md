...
LangGraphを用いた監査エージェント開発では、複数のLLMステップが逐次・並列に実行されるため、各ステップのTTFTとスループットがエージェント全体のレスポンスタイムとコストに直接影響する。このリーダーボードを活用することで、監査エージェントの各ノード（文書解析、リスク評価、レポート生成など）に適切なモデル・プロバイダーを選定する根拠を定量的に得られる。特に「小型モデル並列処理＋大型モデル統合」パターンは、大量の監査証憑を並列スキャンしてGPT-4系で統合判断するアーキテクチャ設計の参考として有用。
