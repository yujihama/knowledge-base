...
Yujiが開発中の監査エージェントシステムをGoogle Cloud上にデプロイする際、Vertex AI Model GardenやGKEとHugging Faceの統合強化により、LangGraphベースのエージェントで使用するオープンモデル（Gemmaなど）の本番環境デプロイが容易になる。またCDN Gatewayによりモデルのダウンロード遅延が減少し、エージェントのiterationサイクルが高速化する恩恵が得られる。セキュリティ面では、Mandiant連携によるモデルスキャンは監査領域におけるAIシステムの信頼性・ガバナンス要件を満たすための参考事例となりうる。
