...
監査エージェントのフロントエンドデモや社内ツールをブラウザで動かす際、Transformers.js v4のWebGPU対応により埋め込みモデル（all-MiniLM等）をサーバレスで推論できる選択肢が生まれる。`ModelRegistry.get_available_dtypes`でq4量子化モデルを選択するなど、リソース制約環境向けの推論最適化手法は、ローカルLLMインフラ（RTX 3090構築中）における量子化戦略の参考になる。ただし本ライブラリはJavaScript/TypeScript環境向けであり、LangGraph/Pythonベースの監査エージェント本体への直接適用は限定的。
