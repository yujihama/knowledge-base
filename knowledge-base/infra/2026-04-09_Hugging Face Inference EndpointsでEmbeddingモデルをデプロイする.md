...
監査エージェントにRAGを組み込む際、監査調書・規程・基準書などのテキストをEmbedding化して意味検索するコンポーネントが必要になる。TEI on Inference Endpointsは450+ req/secのスループットと64倍のコスト優位性を持ち、LangGraphのノード内からHTTP呼び出しで即座に利用可能なため、エージェントのRetrieverノード実装の選択肢として具体的に評価できる。また、VPC接続・SOC2・GDPRへの対応は監査業務で扱う機密データをEmbeddingサービスに流す際のセキュリティ要件を満たす観点からも重要な検討材料となる。
