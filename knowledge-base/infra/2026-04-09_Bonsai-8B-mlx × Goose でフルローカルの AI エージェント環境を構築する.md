...
監査エージェントの開発・評価をオフライン環境で行いたい場面（機密データを含むテストケースでクラウド API に送信できないケース等）に、このフルローカル構成は直接参考になる。LangGraph ベースのエージェントも OpenAI 互換エンドポイントを向ければ Bonsai のようなローカル LLM をバックエンドにできるため、推論コストゼロのローカル評価環境として活用できる。ただし Tool 呼び出しが不安定な点は ReAct ループを多用する監査エージェントでは致命的になるため、function calling の品質を定量評価する LLM-as-judge パイプラインを組み合わせた検証が必要になる。
