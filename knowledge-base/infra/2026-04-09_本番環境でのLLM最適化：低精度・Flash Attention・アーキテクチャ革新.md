...
監査エージェントシステムでは長い監査証跡・規制文書をコンテキストとして渡すケースが多く、KVキャッシュの肥大化とVRAM消費が実際のボトルネックになりえる。建設中のGALLERIA（RTX 3090 24GB予定）では、4bit量子化を用いることで34B〜70Bクラスのモデルをローカル推論できる可能性があり、Ollama等と組み合わせた際のモデル選定指針として直接活用できる。またLangGraphエージェントでLLMを複数ステップ呼び出す際のレイテンシ改善にFlash Attention対応モデルの選択が有効。
