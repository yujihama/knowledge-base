...
監査エージェントシステムでローカルLLM推論基盤（RTX 3090）を検討中のYujiにとって、GPU以外の推論アクセラレータの選択肢としてInferentia2の性能特性（エンコード0.5〜1.7秒、256トークン生成2.3秒）は比較ベースラインになる。RAGユースケースにおける768入力トークンでのエンコード遅延データは、監査エージェントの文書参照フロー設計時のレイテンシ見積もりに直接活用できる。ただし現在のインフラ構成（ローカルGPUサーバ）とはアーキテクチャが異なるため、クラウド展開を検討する段階での参照情報として有用。
