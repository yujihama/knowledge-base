...
監査エージェントシステムでは複数の監査ドキュメントを並列処理するシナリオが多く、連続バッチ処理の理解はローカルLLMインフラ（RTX 3090）上でのvLLM/TGI設定最適化に直結する。特にLangGraphで複数エージェントが並列推論を行う場合、KVキャッシュのメモリ消費モデルを把握しておくことで、バッチサイズとコンテキスト長のトレードオフを設計段階で考慮できる。また、LLM-as-judgeパターンで多数の監査項目を並列評価する際のスループット予測にも本記事の知識が役立つ。
