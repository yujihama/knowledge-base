...
監査エージェント開発においてDeepSeek-R1やLlama-3.2-90B-Vision-InstructをサーバーレスAPIとして手軽に呼び出せるため、LangGraphのノード内でLLM-as-judgeや多段推論を試す際のプロトタイピングコストが下がる。またhuggingface_hubのproviderパラメータで推論バックエンドを差し替えられる設計は、エージェントの評価フェーズでモデルを比較実験する際に有用。ローカルRTX 3090環境が整うまでの暫定推論基盤としても活用できる。
