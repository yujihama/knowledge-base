...
監査エージェント開発で生成した合成データ（Pydanticスキーマ準拠のラベル付き監査事例など）をHugging Face Hubのゲート付きデータセットとして管理することで、組織内での安全な共有とバージョン管理が実現できる。DuckDB SQLコンソールを使えば、RAG用の評価データセットの品質確認やフィルタリングをブラウザ上で即座に行えるため、LLM-as-judgeのベンチマーク構築サイクルを短縮できる。
