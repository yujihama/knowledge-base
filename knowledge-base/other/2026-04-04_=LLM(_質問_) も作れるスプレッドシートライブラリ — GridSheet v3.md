...
監査エージェントの結果をレビュアーが確認・編集するUIとして、スプレッドシート形式は自然な選択肢。GridSheet の =LLM() パターンを応用すれば、監査項目リストを行として並べ、各行でエージェントの評価結果を非同期取得・Spilling で複数観点（リスク区分・根拠・信頼度など）を列展開するUIを比較的少ないコードで実現できる。非同期キャッシュ＋インフライト共有の設計思想は、LangGraph の並列ノードが同一LLMクエリを重複実行するコスト問題への対処としても参考になる。
