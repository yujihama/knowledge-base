...
YujiがDeloitteで開発中の監査エージェントシステムに直接関連する研究。EU AI法コンプライアンス分類はGRC（ガバナンス・リスク・コンプライアンス）業務の自動化そのものであり、LGGT+のニューロシンボリックアプローチはLangGraphベースのエージェントにルールベース推論を組み込む際の実装参考になる。特にT_G（偽陽性許容・高リコール）とT_P（偽陽性ゼロ・保守的）の選択基準は、監査判断における「見落とし重大性」ポリシーに応じた設計判断として実務上有用。ベンチマークデータセット（n=1035）とエンジンがApache 2.0で公開されているため、内部監査エージェントのリスク分類モジュール検証に即活用できる。
